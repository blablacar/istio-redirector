...
const Modal = ({ modal, setModal }) => {
  ...
  const downloadFile = () => {
    const href = window.URL.createObjectURL(modal.payload);
    const link = document.createElement('a');
    link.href = href;
    link.setAttribute('download', 'config.yaml');
    document.body.appendChild(link);
    link.click();
    setModal(defaultPayload)
  }

  return (
    <div className={`${modal.isVisible ? '' : 'hidden'} fixed z-10 inset-0 overflow-y-auto`}>
      <div className="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        >
              ...
              <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                ...
                <div className="mt-2">
                  <p className="text-sm text-gray-500">
                    The Virtual Service has been generated with success !
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
            ...
            ) : modal.payload.PR.length > 0 && (
              <a
                href={modal.payload.PR}
                target="_blank"
                rel="nofollow"
                className="mt-3 w-full inline-flex justify-center rounded-md border border-green-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-green-700 hover:bg-green-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm"
              >
            ...
            <button
              onClick={() => setModal(defaultPayload)}
              type="button"
